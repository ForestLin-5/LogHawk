...
**全链路流程：** log-collector → ingest → RabbitMQ → alerter → 前端

```bash
# 1. 注入 20 条 CRIT 日志（触发"立即告警"规则）
for i in $(seq 1 20); do
  kubectl -n loghawk exec deploy/frontend -- wget -qO- --post-data="[{\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"level\":\"CRIT\",\"service\":\"test-alert\",\"node\":\"k8s-master\",\"message\":\"FATAL: out of memory - test $i\"}]" --header='Content-Type: application/json' http://ingest:8001/ingest
done

# 2. 注入 10 条 ERROR（触发"突发告警"规则）
for i in $(seq 1 10); do
  kubectl -n loghawk exec deploy/frontend -- wget -qO- --post-data="[{\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"level\":\"ERROR\",\"service\":\"api-gateway\",\"node\":\"k8s-master\",\"message\":\"connection timeout $i\"}]" --header='Content-Type: application/json' http://ingest:8001/ingest
done

# 3. 查看 alerter 生成的告警
kubectl -n loghawk exec deploy/alerter -- wget -qO- http://localhost:8004/alerts

# 4. 打开前端 http://<MasterIP>:30080 查看告警面板
```

> 注意：通过 frontend Pod 代理请求，因为 ingest 容器内没有 curl/wget。ingest 容器使用 scratch 镜像，无 shell 工具。

**预期结果：**
- 前端告警中心出现 "CRITICAL: 21 条 CRIT 级别日志" 告警卡片（红色）
- 出现 "ERROR 突发: 1 分钟内 XX 条错误" 告警卡片（橙色）
- 告警统计数字更新，右上角铃铛图标显示数量
- 每条告警标注 "由 LogHawk Alerter 自动触发"

**验证状态（2026-07-29）：** ✅ 已验证通过
