...
### 2026-07-29 — v1.0.1 修复

1. **CRI 日志解析修复** — 修复 log-collector 的 `extractMessage`、`serviceName`、`detectLevel` 三函数
   - `extractMessage`：新增两层 CRI 格式兼容——纯 JSON、CRI-O JSON 带时间戳前缀、CRI 纯文本（SplitN 取第4段）
   - `serviceName`：按 CRI 文件名规范 `<pod>_<ns>_<container>-<hash>.log` 提取容器名
   - `detectLevel`：移除过宽的 `ERR` 子串匹配（误匹配 `stderr` 导致全标 ERROR）
2. **前端时区修复** — 新增 `toBJTime()` 函数，UTC 时间戳自动 +8 小时显示北京时间
3. **告警链路验证** — 通过注入 20 条 CRIT + 10 条 ERROR 日志，验证全链路：log-collector → ingest → RabbitMQ → alerter → 前端告警面板 ✅
4. **DaemonSet 镜像更新陷阱** — 同一 tag 更新时 `imagePullPolicy: IfNotPresent` 不拉新镜像，需换 tag（v1.0.0 → v1.0.1）
...
| CRI 日志全标 ERROR、消息带前缀 | `extractMessage` 未处理 CRI 时间戳头 | SplitN 取第4段 + JSON 解析回退 |
| `detectLevel` 误匹配 `stderr` | `ERR` 子串匹配过宽 | 移除 `ERR` 匹配，仅保留 `ERROR` |
| 服务名带 Pod 后缀 `ingest-xxxxx` | 文件名解析未提取容器名 | 按 CRI 命名规范 `<pod>_<ns>_<container>-<hash>.log` |
| DaemonSet 换镜像不生效 | `IfNotPresent` + 同 tag 不重新拉取 | 升 tag 号（v1.0.0 → v1.0.1） |
| 前端时间显示 UTC 非北京时间 | 直接取 `substring(11,19)` | `toBJTime()` UTC+8 转换 |
| 容器名不匹配 set image 失败 | 各 Deployment 容器名不一致 | 用 `-o jsonpath` 查实际容器名 |
